...
def IVF_train_batch(xp,kmean):
  kmean.fit(xp)
  return kmean.labels_, kmean.cluster_centers_

def IVF_train_alldata(xp,num_part:int=16,iternum:int=64):
     kmeans = KMeans(n_clusters=num_part,max_iter=iternum)
     labels_=[]
     cluster_centers_=[]
     for i in range(5):
      labels_, cluster_centers_=IVF_train_batch(xp[i*1000000:i*1000000+1000000,:],kmeans)
